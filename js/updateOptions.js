var updateOptions = function () {
  ...
  this.init = function () {
    ...
    this.getMessengerState(function (state) {
      $('#messenger-enabled').attr('checked', state)
    })
  }
  ...
  this.setMessengerState = function (state, callback) {
    chrome.storage.sync.set({
      'messenger': state
    }, function () {
      if (callback) {
  ...
  this.getMessengerState = function (callback) {
    chrome.storage.sync.get('messenger', function (items) {
      if (items['messenger'] !== undefined && callback) {
        callback(items['messenger'])
      }
    })
  }
}
